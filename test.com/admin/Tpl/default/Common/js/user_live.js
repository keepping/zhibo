    ...
    function accountDone() {
        ...
        var after_sha_start_upload = $('#after_sha_start_upload').val() ? true : false;
        qcVideo.uploader.init(
                //1: 上传基础条件
                {
                    ...
                    secretId: secret_id, // 云api secretId
                     ...
                     $.ajax({
                         'dataType': 'json',
                         'url': APP_ROOT + 'm.php?m=VideoPlayback&a=sign&args='+encodeURIComponent(argStr),
                         'success': function(d){
                            cb(d['result']);
                        }
                     });
                    ...
                    ,after_sha_start_upload: after_sha_start_upload//sha计算完成后，开始上传 (默认关闭立即上传)
                    ...
                    ,transcodeNotifyUrl: APP_ROOT + "m.php?m=VideoPlayback&a=video_callback"//(转码成功后的回调地址)isTranscode==true,时开启； 回调url的返回数据格式参考  http://www.qcloud.com/wiki/v2/MultipartUploadVodFile
                    ,classId: null
                }
                //2: 回调
                , {
                    ...
                    onFileUpdate: function (args) {
                        ...
                        $line.html(''
                                ...
                                + '<span data-act="del" class="button delete">删除</span>'
                                + finalFileId
                        );
