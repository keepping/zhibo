...
class UserBusinessInvestorAction extends CommonAction{
	public function index(){
		//列表过滤器，生成查询Map对象
		$map = $this->_search ();
		$map['_string']=" (is_investor=2 or is_investor=4 or is_investor=5 or is_investor=6) and investor_status!=1 ";
		//追加默认参数
 	...
 	public function investor_go_allow(){
 		...
 		if($user_data){
 			...
 			if($user_data['v_explain']==''){
 				$user_data['v_explain'] =$user_data['authentication_type'];
 			}
 			$user_data['v_icon'] = get_spec_image(M('AuthentList')->where("name='".trim($user_data['authentication_type']."'"))->getField("icon"));
