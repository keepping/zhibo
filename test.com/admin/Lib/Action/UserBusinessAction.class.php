...
class UserBusinessAction extends CommonAction{
   ...
   public function __construct()
	...
	public function insert() {
		...
		//开始验证有效性
		$this->assign("jumpUrl",u('UserBusiness'."/add"));
	...
	public function update() {
		...
		//开始验证有效性
		$this->assign("jumpUrl",u('User'."/edit",array("id"=>$data['id'])));
...
public function consignee()
	...
	public function userbank_add(){
		...
		$this->assign("back_url",u('User'."/index",array("user_id"=>$user_id)));
		$this->display();
	}
	//银行编辑
	...
	public function userbank_insert() {
		...
		//开始验证有效性
		$this->assign("jumpUrl", u('User'."/add",array("user_id"=>$data['user_id'])) );
	...
	public function userbank_update(){
		B('FilterString');
		$data = M('User')->create ();
		$this->assign("jumpUrl", u('User'."/edit",array("id"=>$data['id'],"user_id"=>$data['user_id'])) );

		$user_bank = M('User')->where("id=".intval($data['id'])." and user_id= ".intval($data['user_id']) )->find();
		if(!$user_bank)
	...
	public function userbank_delete(){
		...
		if (isset ( $id )) {
				...
				$list = M('User')->where ( $condition )->delete();
