...
//        $this->assign('show_bnt',$show_bnt);
 	...
 	public function investor_go_allow()
    {
        ...
        foreach ($user as $k => $v) {
            ...
            $user_id[$k]=$user[$k]['id'];
            $recom[$k]=$user[$k]['family_recom'];
            $family_id[$k]=$user[$k]['family_id'];
        }


            if ($user) {
                for($i=0;$i<count($user);$i++) {
                   $family_recom = $GLOBALS['db']->getOne("select id from " . DB_PREFIX . "family where family_recom='$recom[$i]'");
                    ...
                    if($user_data['v_explain']==''){
                       ...
                       if($family_recom&&$family_id[$i]==0){
                        $data['family_id'] = $family_recom;
                        $data['user_id'] = $user_id[$i];
                        $data['create_time'] = NOW_TIME;//获取当前时间
                        $data['status'] = 1;
                        $data['memo'] = $user_id[$i] . "通过认证加入家族";
                        $GLOBALS['db']->autoExecute(DB_PREFIX . "family_join", $data, "INSERT");
                        $GLOBALS['db']->query("update " . DB_PREFIX . "user set family_id = " . $family_recom . ",family_chieftain = 0 where id=" . $user_id[$i]);
                        $user_redis->update_db($user_id[$i], array('family_id' => $family_recom, 'family_chieftain' => 0));
                       }
                    }
