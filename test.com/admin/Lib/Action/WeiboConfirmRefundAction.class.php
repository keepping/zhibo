...
class WeiboConfirmRefundAction extends CommonAction{
			...
			$m_config =  load_auto_cache("m_config");//初始化手机端配置
				...
				if($refund_data['withdrawals_type'] == 0){
					...
					if (wx_withdraw_cash($id)){
						...
						//写入用户日志
						$data = array();
						$data['weibo_money'] = floatval($refund_data['weibo_money']);
						$data['log_admin_id'] = intval($adm_id);
						$param['type'] = 1;//类型 0表示充值 1表示提现 2赠送道具 3 兑换印票
						$ticket_name = '动态红包';
						$log_msg = '提现'.$data['weibo_money'].$ticket_name.'成功';
						...
						//付款失败解冻用户提现印票
						$sql = "update ".DB_PREFIX."user set weibo_refund_money=weibo_refund_money-".$refund_data['weibo_money']." where  id=".$refund_data['user_id'];
						...
						if($GLOBALS['db']->affected_rows()){
							...
							$param['type'] = 1;//类型 0表示充值 1表示提现 2赠送道具 3 兑换印票
							$ticket_name = '动态红包';
							$log_msg ='提现失败，解冻提现'.$data['weibo_money'].$ticket_name;
                    ...
                    if (intval($_REQUEST['status']) == 1){
                        ...
                        if($GLOBALS['db']->affected_rows()){
                            ...
                            $param['type'] = 1;//类型 0表示充值 1表示提现 2赠送道具 3 兑换印票
                            $ticket_name = '动态红包';
                            $log_msg ='提现失败，解冻提现'.$data['weibo_money'].$m_config['ticket_name'];
                            save_log($refund_data['user_id'].$log_msg,0);
                            account_log_com($data,$user_id,$log_msg,$param);
                        }
                        //写入日志
                        ...
                        $param['type'] = 1;//类型 0表示充值 1表示提现 2赠送道具 3 兑换印票
                        $ticket_name = '动态红包';
                        $log_msg = '提现'.$data['weibo_money'].$m_config['ticket_name'].'成功';
                        account_log_com($data,$refund_data['user_id'],$log_msg,$param);
                        $info = $refund_data['user_id'].$log_msg;
                        save_log($info,1);
                        $this->success("提现成功!");
