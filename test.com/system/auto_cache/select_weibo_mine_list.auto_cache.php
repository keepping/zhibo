...
class select_weibo_mine_list_auto_cache extends auto_cache{
	private $key = "select:mine_list:";
...
//				}
				$sql = "select w.user_id,w.id as weibo_id,u.head_image,u.is_authentication,w.content,w.red_count,w.digg_count,w.comment_count,w.video_count,w.data,u.nick_name,w.sort_num,w.photo_image ,u.city,w.is_top,w.price,w.type,w.create_time,w.city,w.province,w.address from ".DB_PREFIX."weibo as w
					left join ".DB_PREFIX."user as u on w.user_id = u.id where w.user_id =$user_id and w.status = 1 ";
...
//						}
						...
						$list[$k]['weibo_place'] = $v['province'].$v['city'].$v['address'];
						if($v['type']=='video'){
							$list[$k]['images'] = array();
							$url = $v['data'];
