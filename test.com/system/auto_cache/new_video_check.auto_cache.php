...
class new_video_check_auto_cache extends auto_cache{
	private $key = "new:video:check:";
		...
		if ($list === false) {
			...
			if(!$is_ok){
				...
				$sql = "SELECT v.id AS room_id, v.sort_num, v.group_id, v.user_id, v.city, v.title, v.cate_id, v.live_in, v.video_type, v.create_type, v.room_type,
					...
					LEFT JOIN ".DB_PREFIX."user u ON u.id = v.user_id where v.live_in in (1,3) and v.room_type = 3 and (u.mobile = '13888888888' or u.mobile = '13999999999')";
