    ...
    function connect_readdb($charset = 'utf8')
    {
    	if ($this->query_link != null){
    		...
    		if($c==0){
    			...
    			if (PHP_VERSION >= '4.2')
    			{
    				$link_db = @mysql_connect($dbhost, $dbuser, $dbpw, true);
    			}
    			else
    			{
    				$link_db = @mysql_connect($dbhost, $dbuser, $dbpw);
    			}
    			if($link_db)
    			{
    				$this->version = mysql_get_server_info($link_db);
    				...
    				if ($this->version > '4.1')
    				{
    					...
    					{
    						mysql_query("SET sql_mode=''", $link_db);
    					}
    				}
    				if ($dbname)
    				{
    					if (mysql_select_db($dbname, $link_db) === false )
    					{
    						@mysql_close($link_db);
    						$link_db = null;
    					}
