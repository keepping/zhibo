		...
		if($wx_info['unionid']!=""||$wx_info['openid']!=""){
			...
			if(!$has_user){
					...
					if($wx_info['headimgurl']!=''){
						...
						//兼容无法获取图片
						if(!file_exists($image_file_domain)||filesize($image_file)==0){
							getImage($wx_info['headimgurl'],$root['savePath'],$save_name,1);
						}
					...
					if(intval($GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where wx_unionid = '".$wx_info['unionid']."' and login_type = 0 "))==0){
